...
router.post("/contactUs", async (req, res) => {
  try {
    const mailData = {
      from: req.body.email,
      to: "michael@360ground.com",
      subject: `contact us message from ${req.body.firstName} ${req.body.lastName}, phone number : ${req.body.phoneNumber}`,
      text: req.body.message,
    };

    transporter.sendMail(mailData, async function (err, data) {
      if (err) {
        res.status(200).send({ status: false, message: "Message not sent" });
      } else {
        res.status(200).send({
          status: true,
          message:
            "Thank you for contacting us. We will reach you as soon as we can.",
        });
      }
    });
  } catch (error) {
...
router.post("/createImage", async (req, res) => {
  ...
  let result = await convertBase64(
    image,
    req.body.id,
    `images/slide`,
    true,
...
router.post("/deleteImage", async (req, res) => {
  ...
  try {
    await req.body.images.forEach(async (element) => {
      let result = await deleteFiles(
        { body: { url: element.url } },
        res,
        false
      );
    ...
    if (ids.length) {
      pool.get_connection((qb) => {
        qb.delete("tbl_images_gallery", { id: ids }, (err) => {
          ...
          res
            .status(200)
            .send({ status: true, message: "Image deleted successfully." });
        });
      });
    } else {
      res.status(200).send({
        status: false,
        message: "Unable to delete Images. try again.",
      });
    }
  } catch (err) {
    res.status(200).send({ status: false, message: err.message });
