    ...
    requestPromise(requestOption)
      .then(async function (response) {
        ...
        if (response.status == "created") {
          ...
          let result = deletePaymentSideeffect(
            req.body.data.metaData.paymentId
          );

          res.status(200).send({
            status: result.status,
            message:
              "unable to process the payment now. please try again later.",
          });
        }
      })
...
router.post("/paymentSuccessCallBack", async (req, res) => {
  try {
    ...
    if (req.body.status == "PAYED") {
      ...
      let result = deletePaymentSideeffect(
        req.body.metaData.metaData.paymentId
      );
