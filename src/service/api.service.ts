...
export class ApiService {
  ...
  main(data: any) {
    const httpOptions = {
      headers: new HttpHeaders({
        ...
        // You can include other CORS headers as needed, such as Access-Control-Allow-Methods, Access-Control-Allow-Headers, etc.
      }),
    };
    return this.http.post(environment.baseUrl, data, httpOptions);
  }
  ...
  mainCanvas(endpoint: any, method: any, data: any): any {
    ...
    if (method == 'get') {
      return this.http.get(
        `${environment.baseUrlBackend}/${endpoint}`,
        httpOptions
      );
    } else if (method == 'post') {
  ...
  getEnrolledCourses(userId: any) {
    ...
    this.mainCanvas(`getUserEnrollment/${userId}`, 'get', {}).subscribe(
      (enrollments: any[]) => {
        this.mainCanvas(`getAllEnrolledCourses/${userId}`, 'get', {}).subscribe(
          async (courses: any[]) => {
            enrollments.forEach((enrollment: any) => {
              moduleRequests.push(
                this.mainCanvas(
                  `getAllModules/${enrollment.course_id}?studentId=${this.userData.id}`,
                  'get',
                  {}
                )
              );
          ...
          modules.items.forEach((item: any) => {
            if (item.type == 'Quiz') {
              if (!item.completion_requirement?.completed) {
                ...
                QuizzesSubmissionRequests.push(
                  this.mainCanvas(
                    `getQuizSubmission/${course.courseId}/${item.id}`,
                    'get',
                    {}
                  )
                );
      ...
      responses.forEach((element: any, index: any) => {
        if (element.status) {
          element.message.forEach((datas: any) => {
            let data = datas.find((da: any) => {
              return da.user_id == this.userData.id;
            });
          });
        } else {
  ...
  separate(data: any, userId: any) {
    ...
    this.mainCanvas(`getAllCertificates/${userId}`, 'get', {}).subscribe(
      (response: any) => {
        ...
        data.forEach((element: any, index: any) => {
          ...
          if (location.protocol == 'http:') {
            element.image_download_url = element.image_download_url.replace(
              'https',
              'http'
            );
          }
          ...
          if ('requirement_count' in progress) {
            element.percentage =
              (progress.requirement_completed_count /
            ...
            if (
              ...
              let certificate = this.MyCertificates.find(
                (certi: any) => +certi.courseId == element.id
              );
              ...
              if (certificate !== undefined) {
                ...
                element.end_at
                  ? (payload.coureseEndDate = element.end_at)
                  : null;
  ...
  checkCourseEnrollment(courseId: any) {
    ...
    if (this.myCourses.merged) {
      index = this.myCourses.merged.findIndex(
        (ele: any) => ele.id == +courseId
      );
    }
  ...
  loadCourses(limit: any) {
    this.mainCanvas(
      `getAllCourseExtraInfo/${limit}/published`,
      'get',
      null
    ).subscribe((response: any) => {
      if (response.status) {
        ...
        message.forEach((element: any) => {
          ...
          if (location.protocol == 'http:') {
            if (element.attributes.image_download_url) {
              element.attributes.image_download_url =
                element.attributes.image_download_url.replace('https', 'http');
            }
          }
        });
